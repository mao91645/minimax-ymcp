...
from minimax_ymcp.utils import (
    build_output_path,
    build_output_file,
    process_input_file,
    play
...
from minimax_ymcp.const import *
from minimax_ymcp.exceptions import MinimaxAPIError, MinimaxRequestError
from minimax_ymcp.client import MinimaxAPIClient
...
mcp = FastMCP("Minimax-Y", log_level=fastmcp_log_level)
...
# ── Web Search & Image Understanding (from minimax-coding-plan) ─────────────

def _process_image_url(image_source: str) -> str:
    """Convert image_source to data URL."""
    ...
    # local file (strip leading @)
    file_path = image_source.lstrip("@")
    if not os.path.isabs(file_path):
        raise MinimaxRequestError(f"Local file path must be absolute: {file_path}")
    with open(file_path, "rb") as f:
        data = f.read()
    ext = Path(file_path).suffix.lower()
    mime_type = "jpeg" if ext in (".jpg", ".jpeg") else ext.lstrip(".")
    return f"data:image/{mime_type};base64,{base64.b64encode(data).decode('utf-8')}"


@mcp.tool(description="""Search the web for real-time or external information.
    You MUST use this tool whenever you need to search for real-time or external information on the web.

    Args:
        query (str): The search query. Aim for 3-5 keywords for best results.
            For time-sensitive topics, include the current date.
...
@mcp.tool(description="""Analyze, describe, or extract information from an image using AI vision.
    ...
    Args:
        ...
        image_source (str): The location of the image to analyze.
            ...
            - Local file path (absolute path, leading @ is optional)
...
def understand_image(prompt: str, image_source: str) -> TextContent:
    ...
    processed_url = _process_image_url(image_source)
    ...
    if not content:
        ...
        return TextContent(
            type="text",
            text="Understand Image Failed: No content returned from VLM API, error:\n" +
                 json.dumps(response_data.get("base_resp", {}), ensure_ascii=False, indent=2)
        )
...
def main():
    print("Starting Minimax-Y MCP server — 11 tools")
